...
def main():
    ...
    base_nutrition_tensor = torch.tensor(
    ...
    print(
        f"Simulating {len(scenarios)} scenarios over the last {simulation_days} days..."
    )
    ...
    ax.plot(
        time_index[plot_range],
        results["control"][plot_range] * 1000,
        label="Control (Original Data)",
        color="k",
        linestyle="--",
        linewidth=2,
    )
    ax.plot(
        time_index[plot_range],
        results["high_salt"][plot_range] * 1000,
        label="High Salt Scenario",
        color="r",
        linewidth=2.5,
        alpha=0.8,
    )
    ax.plot(
        time_index[plot_range],
        results["high_carbs"][plot_range] * 1000,
        label="High Carbs Scenario",
        color="b",
        linewidth=2.5,
        alpha=0.8,
    )
    ax.plot(
        time_index[plot_range],
        results["high_water"][plot_range] * 1000,
        label="High Water Intake Scenario",
        color="g",
        linewidth=2.5,
        alpha=0.8,
    )
    ...
    ax.set_title(
        "Sensitivity Analysis: How Nutrition Affects Predicted Base Metabolism",
        fontsize=16,
        pad=20,
    )
